...
int main() {
    ...
    for (int i = 0; i < 10; ++i) {
        ...
        // 2-opt
        // best_order = two_opt(coords, best_order);
        // int min_r_2opt = calculate_route(coords, best_order);

        // 2.5-opt
        best_order = two_and_half_opt(coords, best_order);
        int min_r_25opt = calculate_route(coords, best_order);

        // // 3-opt
        // best_order = three_opt(coords, best_order, MAX_ITERATIONS);
        // int min_r_3opt = calculate_route(coords, best_order);

        // std::cout << files[i] << " 2-opt: " << min_r_2opt <<" Best known: " << best_known_route << std::endl;
        std::cout << files[i] << " 2.5-opt: " << min_r_25opt <<" Best known: " << best_known_route << std::endl;
        // std::cout << files[i] << " 3-opt: " << min_r_3opt <<" Best known: " << best_known_route << std::endl;
    }
